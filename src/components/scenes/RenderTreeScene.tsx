    ...
    <group position={[x, y, 0]}>
      <mesh ref={ref}>
        ...
        <meshStandardMaterial
          color={hidden ? '#FF4D6D' : '#0D0D1A'}
          emissive={hidden ? '#FF4D6D' : color}
      ...
      <lineSegments geometry={edgeGeo}>
        <lineBasicMaterial color={hidden ? '#FF4D6D' : color} transparent opacity={0.4} />
      </lineSegments>
      <Text position={[0, 0, 0.05]} fontSize={0.11} color={hidden ? '#FF4D6D' : color} anchorX="center" anchorY="middle">{label}</Text>
    </group>
  )
}
...
function MergeArrow() {
  ...
  return (
    <group position={[0.2, 0, 0]}>
      <line geometry={geo}>
        <lineBasicMaterial color="#FFB340" />
      </line>
      <Text position={[0, 0.35, 0]} fontSize={0.13} color="#FFB340" anchorX="center" anchorY="middle">merge</Text>
      <mesh ref={ref} position={[1.3, 0, 0]} rotation={[0, 0, -Math.PI / 2]}>
        <coneGeometry args={[0.08, 0.2, 8]} />
        <meshStandardMaterial color="#FFB340" emissive="#FFB340" emissiveIntensity={0.5} />
      </mesh>
    </group>
  )
}
...
function HiddenLabel() {
  ...
  return (
    <group ref={ref} position={[-4, -0.6, 0.2]}>
      <Text position={[0, 0.5, 0]} fontSize={0.11} color="#FF4D6D" anchorX="center" anchorY="middle">omitted!</Text>
    </group>
  )
}
...
export default function RenderTreeScene() {
  return (
    <Canvas camera={{ position: [0, 0, 9], fov: 50 }} gl={{ antialias: true, alpha: true }} style={{ width: '100%', height: '100%' }}>
      ...
      {DOM_NODES.map(n => <TreeNode key={n.label} {...n} color="#4D9FFF" />)}
      {CSSOM_NODES.map(n => <TreeNode key={n.label} {...n} color="#00E5A0" />)}
      {RENDER_NODES.map(n => <TreeNode key={n.label} {...n} color="#FFB340" />)}
      <MergeArrow />
      <HiddenLabel />
      <Text position={[-4.5, 2.0, 0]} fontSize={0.18} color="#4D9FFF" anchorX="center" anchorY="middle">DOM</Text>
      <Text position={[-1.5, 2.0, 0]} fontSize={0.18} color="#00E5A0" anchorX="center" anchorY="middle">CSSOM</Text>
      <Text position={[3.0, 2.0, 0]} fontSize={0.18} color="#FFB340" anchorX="center" anchorY="middle">Render Tree</Text>
    </Canvas>
  )
}
